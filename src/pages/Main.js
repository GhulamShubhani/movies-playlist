...
const Main = () => {
  ...
  const { email, gender } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    const getAllData = async () => {
      try {
        const { data } = await axios.get(
          "https://smoggy-necklace-yak.cyclic.app/playlist/getplaylist",
          config
        );
  ...
  const SubmitApiCall = async () => {
    if (selectExisting === "old") {
      ...
      try {
        ...
        const res = await axios.post(
          "http://192.168.1.6:8000/playlist/CreatePlaylist",
          {
  ...
  const handleSearch = (e) => {
    ...
    if (searchValue.length === 0 || searchValue===null) {
      // If the search input is empty, reset the data to display all items
      getAllData();
    } else {
  ...
  return (
    <Container>
      <Grid container>
        ...
        >
          <DialogTitle id="alert-dialog-title">{"Playlist"}</DialogTitle>
          <DialogContent>
                    ...
                    >
                      <Grid item xs={12} sm={6}>
                        <TextField
                          ...
                          onChange={(e) => setplaylistEwName(e.target.value)}
                          required
                          fullWidth
                          id="playlistEwName"
                          label="Playlist Name"
